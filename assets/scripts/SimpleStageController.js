...
cc.Class({
    ...
    resolveSceneReferences() {
        ...
        this.scoreDigits = this.scoreLabel && this.scoreLabel.string
            ...
            : 7;
        this.resolveLevelClearResultLabels();
    },
    ...
    resolveLevelClearResultLabels() {
        this.levelClearResultRoot = null;
        this.clearResultTimerLabel = null;
        this.clearResultMultiplierLabel = null;
        this.clearResultBonusLabel = null;

        const resultRoot = this.findLevelClearResultRoot();
        if (!resultRoot || !cc.isValid(resultRoot)) {
            return;
        }

        this.levelClearResultRoot = resultRoot;
        this.clearResultTimerLabel = this.getLabelFromNodeName(resultRoot, 'time')
            || this.getLabelFromNodeName(resultRoot, 'timer');
        this.clearResultMultiplierLabel = this.getLabelFromNodeName(resultRoot, 'multipler')
            || this.getLabelFromNodeName(resultRoot, 'multiplier');
        this.clearResultBonusLabel = this.getLabelFromNodeName(resultRoot, 'Final Score');
    },

    findLevelClearResultRoot() {
        const scene = cc.director.getScene();
        if (!scene || !scene.getComponentsInChildren) {
            return null;
        }

        const goalPoles = scene.getComponentsInChildren('GoalPole') || [];
        for (let i = 0; i < goalPoles.length; i += 1) {
            const goalPole = goalPoles[i];
            if (goalPole && goalPole.resultRoot && cc.isValid(goalPole.resultRoot)) {
                return goalPole.resultRoot;
            }
        }
    ...
    getLabelFromNodeName(parent, nodeName) {
        if (!parent || !nodeName || !parent.children) {
            return null;
        }
        ...
        for (let i = 0; i < parent.children.length; i += 1) {
            ...
            const nestedLabel = this.getLabelFromNodeName(child, nodeName);
            if (nestedLabel) {
                return nestedLabel;
            }
        }
    ...
    ensureLevelClearResultLabels() {
        const hasTimerLabel = this.clearResultTimerLabel && cc.isValid(this.clearResultTimerLabel);
        const hasMultiplierLabel = this.clearResultMultiplierLabel && cc.isValid(this.clearResultMultiplierLabel);
        const hasBonusLabel = this.clearResultBonusLabel && cc.isValid(this.clearResultBonusLabel);

        if (hasTimerLabel && hasMultiplierLabel && hasBonusLabel) {
            return;
        }
    ...
    clearLevel() {
        ...
        const clearTime = this.remainingTime;
        const timeBonus = clearTime * this.clearTimeBonusMultiplier;
        this.levelCleared = true;
        if (this.sceneName === this.stage1SceneName) {
            this.stage1Cleared = true;
        }
        this.pauseStageBgm();
        this.updateLevelClearResultUi(clearTime, this.clearTimeBonusMultiplier, timeBonus);
    ...
    updateLevelClearResultUi(clearTime, multiplier, timeBonus) {
        this.ensureLevelClearResultLabels();

        if (this.clearResultTimerLabel) {
            this.clearResultTimerLabel.string = String(this.sanitizeNumber(clearTime, 0));
        }

        if (this.clearResultMultiplierLabel) {
            this.clearResultMultiplierLabel.string = String(this.sanitizeNumber(multiplier, 0));
        }

        if (this.clearResultBonusLabel) {
            this.clearResultBonusLabel.string = String(this.sanitizeNumber(timeBonus, 0));
        }
    }
});
